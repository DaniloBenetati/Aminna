                    ...
                    <div className="flex items-center gap-1">
                        <span className={`w-2 h-2 rounded-full ${appointment.status === 'Confirmado' ? 'bg-emerald-500' :
                            appointment.status === 'Em Andamento' ? 'bg-blue-500' :
                                appointment.status === 'Concluído' ? 'bg-slate-500' :
                                    'bg-amber-400'
                            }`}></span>
                        {appointment.status === 'Concluído' && (
                            (() => {
                                const record = nfseRecords.find(r => r.appointmentId === appointment.id);
...
interface DroppableSlotProps {
    ...
    zoomLevel: number;
    rowHeight: number;
    onNewAppointment: (data: { providerId: string; date: string; time: string }) => void;
    gridDateStr: string;
    children: React.ReactNode;
}

const DroppableSlot: React.FC<DroppableSlotProps> = ({ id, hour, providerId, zoomLevel, rowHeight, onNewAppointment, gridDateStr, children }) => {
    const { isOver, setNodeRef } = useDroppable({
        id,
        data: { providerId, hour }
    });

    return (
        <div
            ref={setNodeRef}
            className={`flex-shrink-0 border-r border-slate-50 dark:border-zinc-800 p-1 relative group transition-all duration-300 ${isOver ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : 'hover:bg-slate-50/50 dark:hover:bg-zinc-800/30'}`}
...
export const Agenda: React.FC<AgendaProps> = ({
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const { providerId, hour } = over.data.current as { providerId: string; hour: string };

            if (!appointment || !providerId || !hour) return;

            // Update locally first
            const oldProviderId = appointment.providerId;
            const oldTime = appointment.time;

            if (oldProviderId === providerId && oldTime === hour) return;

            setAppointments(prev => prev.map(a =>
                a.id === appointment.id
                    ? { ...a, providerId, time: hour }
                    : a
            ));
            ...
            if (error) {
                ...
                setAppointments(prev => prev.map(a =>
                    a.id === appointment.id
                        ? { ...a, providerId: oldProviderId, time: oldTime }
                        : a
                ));
            }
        }
    ...
    return (
        <DndContext
            sensors={sensors}
            onDragStart={(event) => setDraggedAppt(event.active.data.current?.appointment)}
        ...
        >
            ...
            <DragOverlay style={{ pointerEvents: 'none' }}>
                {draggedAppt ? (
                    <div className={`p-1.5 rounded-xl border text-left shadow-2xl bg-indigo-600 border-indigo-700 text-white w-32 opacity-90 scale-105 transition-transform`}
                    >
                        <div className="flex justify-between items-start">
                            <span className="text-[9.5px] font-black uppercase truncate">{customers.find(c => c.id === draggedAppt.customerId)?.name.split(' ')[0]}</span>
                        </div>
                        <div className="text-[8.5px] font-bold truncate mt-0.5">{services.find(s => s.id === draggedAppt.serviceId)?.name}</div>
                    </div>
                ) : null}
            </DragOverlay>
        </DndContext>
