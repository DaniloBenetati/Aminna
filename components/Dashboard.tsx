...
import { ViewState, Customer, Appointment, Sale, StockItem, Service, Campaign } from '../types';
import { PROVIDERS, PARTNERS } from '../constants';
...
export const Dashboard: React.FC<DashboardProps> = ({ appointments, customers, sales, stock, services, campaigns }) => {
  // --- FILTER STATES ---
  ...
  const topProducts = useMemo(() => {
      ...
      filteredSales.forEach(s => {
          revenue[s.productId] = (revenue[s.productId] || 0) + s.totalPrice;
      });
      return Object.entries(revenue)
          .map(([id, val]) => ({
              name: stock.find(p => p.id === id)?.name || 'Produto Removido',
              value: val
          }))
          .sort((a, b) => b.value - a.value)
          .slice(0, 5);
  ...
  }, [currentMonthIdx, timeView, dateRef]);
