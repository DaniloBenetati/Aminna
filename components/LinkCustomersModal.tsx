...
export const LinkCustomersModal: React.FC<LinkCustomersModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-sm border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="p-6 md:p-8 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 rounded-sm">
              <Users size={24} />
            </div>
            <div>
        ...
        <div className="p-6 md:p-8 space-y-6 max-h-[60vh] overflow-y-auto">
          ...
          {/* Alerta explicativo */}
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-sm p-4 flex gap-3 text-amber-800 dark:text-amber-300">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                disabled={isMerging}
                className="w-full bg-slate-50 dark:bg-zinc-900 border-2 border-slate-200 dark:border-zinc-800 rounded-sm pl-11 pr-4 py-3.5 text-sm font-black text-slate-900 dark:text-white outline-none focus:border-indigo-500 placeholder:text-slate-400"
                value={searchTerm}
              ...
              {searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-sm shadow-xl z-30 overflow-hidden divide-y divide-slate-100 dark:divide-zinc-700">
                  {searchResults.map(c => (
                    <button
                      key={c.id}
                      type="button"
            ...
            <div className="space-y-2.5">
              {selectedCustomers.map(c => {
                ...
                return (
                  <div 
                    key={c.id} 
                    className={`p-4 rounded-sm border-2 transition-all flex items-center justify-between gap-3 ${
                      isMain 
                  ...
                  >
                    ...
                    {!isCurrent && (
                      <button
                        ...
                        className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-sm transition-all disabled:opacity-50"
                      >
                        <X size={16} />
                      </button>
                    )}
          ...
          {selectedCustomers.length >= 2 && (
            <div className="bg-slate-50 dark:bg-zinc-800/40 border border-slate-100 dark:border-zinc-800 rounded-sm p-5 space-y-3">
        ...
        <div className="p-6 md:p-8 bg-slate-50/50 dark:bg-zinc-900/50 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isMerging}
            className="px-5 py-3 border-2 border-slate-200 dark:border-zinc-700 text-slate-600 dark:text-slate-300 rounded-sm text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all hover:bg-slate-100 dark:hover:bg-zinc-800 disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-6 py-3.5 bg-indigo-600 text-white rounded-sm text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/10 active:scale-95 transition-all flex items-center gap-2 hover:bg-indigo-700 disabled:opacity-50 disabled:bg-slate-300 dark:disabled:bg-zinc-800 disabled:text-slate-400 disabled:shadow-none"
          >
            {isMerging ? (
              <>
        ...
        <div className="fixed inset-0 bg-slate-950/70 dark:bg-black/90 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-[2.2rem] border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 space-y-4">
              ...
              {/* Ícone de alerta */}
              <div className="w-12 h-12 bg-amber-50 dark:bg-amber-950/30 text-amber-500 rounded-sm flex items-center justify-center mx-auto">
              ...
              {/* Caixa de detalhes */}
              <div className="bg-slate-50 dark:bg-zinc-800/40 rounded-sm p-4 text-xs font-bold text-slate-600 dark:text-slate-300 space-y-3.5 border border-slate-100 dark:border-zinc-800">
                <div>
                ...
                <div className="border-t border-slate-150 dark:border-zinc-700 pt-3">
                  ...
                  <div className="space-y-1.5 mt-1">
                    {selectedCustomers.filter(c => c.id !== mainCustomerId).map(c => (
                      <div key={c.id} className="flex justify-between items-center bg-white dark:bg-zinc-850 p-2.5 rounded-sm border border-slate-100 dark:border-zinc-800 shadow-sm">
            ...
            <div className="p-4 bg-slate-50/50 dark:bg-zinc-900/50 border-t border-slate-100 dark:border-zinc-800 flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-white dark:bg-zinc-800 border-2 border-slate-200 dark:border-zinc-700 text-slate-600 dark:text-slate-300 rounded-sm text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 py-3 bg-indigo-600 text-white rounded-sm text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/10 active:scale-95 transition-all hover:bg-indigo-700"
              >
                Confirmar
              </button>
            </div>
