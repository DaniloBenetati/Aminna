...
export const Closures: React.FC<ClosuresProps> = ({ services, appointments, providers, customers }) => {
  ...
  const getFinancials = (providerId: string) => {
    ...
    appointments.forEach(app => {
      ...
      if (app.providerId === providerId) {
        ...
        // FIXED: Subtract tips from pricePaid to get true service base value
        if ((!app.additionalServices || app.additionalServices.length === 0) && app.pricePaid && app.pricePaid > 0) {
          baseValue = app.pricePaid - (app.tipAmount || 0);
        }
    ...
    return { provider, revenue, commissionRate: effectiveRate, commissionVal, serviceCommission, totalTips, count, details: detailedServices };
  };
  ...
  const totalServiceCommission = reportData.reduce((acc, curr) => acc + curr.serviceCommission, 0);
  const totalRevenue = reportData.reduce((acc, curr) => acc + curr.revenue, 0);
  const totalTips = reportData.reduce((acc, curr) => acc + curr.totalTips, 0);
          ...
          <div className="flex-1 flex flex-col justify-start pt-4">
            <div className="mb-12 relative pl-8 border-l-2 border-slate-900">
              ...
              <p className="text-[10px] text-slate-500 mt-4">
                Composição: Comissões (R$ {data.serviceCommission.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}) + Caixinhas (R$ {data.totalTips.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})
              </p>
            </div>
  ...
  const generateWhatsappMessage = (data: any) => {
    ...
    message += `*FATURAMENTO:* R$ ${data.revenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}\n`;
    message += `*VLR P/ NOTA (SERVIÇOS):* R$ ${data.serviceCommission.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}\n`;
    message += `*CAIXINHAS (100%):* R$ ${data.totalTips.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}\n\n`;
    message += `*TOTAL A REPASSAR: R$ ${data.commissionVal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}*\n\n`;
    ...
    <div className="space-y-6 relative h-full flex flex-col pb-24 md:pb-8 text-slate-900 dark:text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[1.5rem] border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
          <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Faturamento Bruto</p><p className="text-xl font-black">R$ {totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
          <div className="bg-slate-50 dark:bg-zinc-800 p-3 rounded-xl text-slate-400"><Files size={20} /></div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[1.5rem] border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
          <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total para Nota</p><p className="text-xl font-black text-indigo-600">R$ {totalServiceCommission.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
          <div className="bg-slate-50 dark:bg-zinc-800 p-3 rounded-xl text-indigo-400"><FileText size={20} /></div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[1.5rem] border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
          <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Caixinhas</p><p className="text-xl font-black text-rose-600">R$ {totalTips.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/10 p-5 rounded-[1.5rem] border border-emerald-100 shadow-sm flex items-center justify-between">
          <div><p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1">Total a Repassar</p><p className="text-xl font-black text-emerald-700">R$ {totalToPay.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-3xl border border-emerald-100">
                  ...
                  <p className="text-sm font-black text-emerald-700">R$ {selectedReceipt.commissionVal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
