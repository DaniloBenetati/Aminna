      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 overflow-hidden">
        <table className="w-full text-left">
          <thead><tr className="bg-slate-50 dark:bg-zinc-800">
            ...
            <th className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase">Link</th>
          </tr></thead>
          <tbody>{(r?.data || []).map((p, i) => (
            <tr key={p.id} className="border-b border-slate-50 dark:border-zinc-800">
              ...
              <td className="px-4 py-2">{p.permalink && <a href={p.permalink} target="_blank" rel="noreferrer" className="text-[9px] text-indigo-500 hover:underline">Ver ↗</a>}</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
