        ...
        <div className="flex flex-col h-full bg-slate-50/50 dark:bg-zinc-950/50">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 md:p-8 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 shrink-0 gap-4">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-slate-100 dark:bg-zinc-800 rounded-sm flex items-center justify-center border border-slate-200 dark:border-zinc-700 shadow-inner">
                ...
                <button 
                    ...
                    className="p-3 bg-white dark:bg-zinc-900 text-slate-600 dark:text-slate-300 border-2 border-slate-200 dark:border-zinc-800 rounded-sm hover:border-slate-900 transition-all font-black uppercase tracking-widest text-[10px] flex gap-2 active:scale-95 shadow-sm"
                >
            ...
            <div className="flex-1 overflow-auto p-4 md:p-8">
                {/* Tabs */}
                <div className="flex bg-white dark:bg-zinc-900 p-2 rounded-sm border-2 border-slate-100 dark:border-zinc-800 mb-6 gap-2 overflow-x-auto scrollbar-hide shadow-sm max-w-fit">
                    {(['NOVAS', 'AGUARDANDO', 'FINALIZADAS', 'CANCELADAS'] as const).map(tab => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-3 rounded-sm text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap ${
                                    activeTab === tab 
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                        {isLoading ? (
                            ...
                            <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-sm border-2 border-dashed border-slate-200 dark:border-zinc-800 shadow-sm">
                            ...
                            filteredReservations.map(res => (
                                <button
                                    ...
                                    className={`w-full text-left bg-white dark:bg-zinc-900 border-2 rounded-sm p-5 md:p-6 transition-all hover:shadow-xl active:scale-[0.99] group ${
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                        <div>
                                            <div className="flex items-center gap-3 mb-2">
                                                <span className={`px-3 py-1 rounded-sm text-[10px] font-black uppercase tracking-widest border ${getStatusColor(res.status)}`}>
                                                    {res.status}
                                                </span>
                    ...
                    <div className="lg:col-span-1">
                        {selectedReservation ? (
                            <div className="bg-white dark:bg-zinc-900 rounded-sm border-2 border-slate-100 dark:border-zinc-800 p-6 sticky top-8 shadow-xl">
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <span className={`px-3 py-1 rounded-sm text-[10px] font-black uppercase tracking-widest border ${getStatusColor(selectedReservation.status)}`}>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="space-y-3">
                                        {selectedReservation.items?.map((item: any) => (
                                            <div key={item.id} className="flex justify-between items-center text-xs p-3 bg-slate-50 dark:bg-zinc-800/50 rounded-sm border border-slate-100 dark:border-zinc-800">
                                ...
                                <div className="flex justify-between items-center bg-slate-100 dark:bg-zinc-800 p-4 rounded-sm mb-6">
                                ...
                                {onConvertToSale && (selectedReservation.status === 'Pendente' || selectedReservation.status === 'Aprovada') && (
                                    <button 
                                        ...
                                        className="w-full flex items-center justify-center gap-2 p-4 bg-indigo-600 text-white rounded-sm text-xs font-black uppercase tracking-widest hover:bg-indigo-700 active:scale-95 transition-all shadow-md shadow-indigo-500/20 mb-4"
                                    >
                                ...
                                <div className="space-y-2">
                                    {selectedReservation.status === 'Pendente' && (
                                        <>
                                            <button 
                                                ...
                                                className="w-full flex items-center justify-center gap-2 p-4 bg-emerald-500 text-white rounded-sm text-xs font-black uppercase tracking-widest hover:bg-emerald-600 active:scale-95 transition-all shadow-md shadow-emerald-500/20"
                                            >
                                                <Check size={16} /> Aprovar / Aguardando
                                            </button>
                                            <button 
                                                onClick={() => updateStatus(selectedReservation.id, 'Rejeitada')}
                                                className="w-full flex items-center justify-center gap-2 p-4 bg-white border-2 border-rose-200 text-rose-500 rounded-sm text-xs font-black uppercase tracking-widest hover:bg-rose-50 hover:border-rose-300 active:scale-95 transition-all"
                                            >
                                    ...
                                    {selectedReservation.status === 'Aprovada' && (
                                        <>
                                            <button 
                                                ...
                                                className="w-full flex items-center justify-center gap-2 p-4 bg-blue-500 text-white rounded-sm text-xs font-black uppercase tracking-widest hover:bg-blue-600 active:scale-95 transition-all shadow-md shadow-blue-500/20"
                                            >
                                    ...
                                    {selectedReservation.status === 'Rejeitada' && (
                                        <button 
                                            ...
                                            className="w-full flex items-center justify-center gap-2 p-4 bg-slate-100 text-slate-500 rounded-sm text-[10px] font-black uppercase tracking-widest hover:bg-rose-50 hover:text-rose-500 active:scale-95 transition-all"
                                        >
                            ...
                            <div className="hidden lg:flex bg-slate-50 dark:bg-zinc-900 border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-sm h-64 items-center justify-center p-6 text-center">
